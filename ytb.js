// ==UserScript==
// @name           YouTube Adblock by Duka
// @name:de        YouTube Adblock by Duka
// @namespace      yt-adblock-duka
// @version        1.4.3
...
(() => {
    ...
    log("By Duka - https://dulkith.com - Code: https://github.com/dulkith/YtAdblock", true);
    ...
    const observer2 = new MutationObserver(checkYtIconExistence);

    // Function to be called when the target element is found
    function modifyYtIcon(ytIcons) {
        ytIcons.forEach(ytIcon => {
            const img = document.createElement('img');
            img.src = 'https://i.ibb.co/G2j7XHY/duka-logo-jwplayer.png'; // Replace with the URL to your PNG image
            img.setAttribute('alt', 'YouTube Logo');
            img.style.width = '100%';
            img.style.height = '100%';

            const parent = ytIcon.parentNode;
            parent.replaceChild(img, ytIcon);
        });

        // Disconnect the observer once the element is found
        observers2.disconnect();
    }

    // Function to check if the target element exists and call the modification function
    function checkYtIconExistence() {
        const ytIcons = document.querySelectorAll('ytd-topbar-logo-renderer svg');
        if (ytIcons.length > 2) {
            modifyYtIcon(ytIcons);
        };
    }
